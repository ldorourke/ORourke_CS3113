...
void placeEntity(const std::string& type, float x, float y) {
	if (type == "Player" || type == "player") { player = Entity(x, y, 256, type); }
	else if (type == "Goal" || type == "goal") { goal = Entity(x, y, 145, type); }
	else if (type == "Enemy" || type == "enemy") { enemies.push_back(new Entity(x, y, 258, type)); }
...
bool readHeader(std::ifstream &stream) {
    ...
    if (mapWidth == -1 || mapHeight == -1) { return false; }
		...
		for (int i = 0; i < mapHeight; ++i) { levelData[i] = new int[mapWidth]; }
		return true;
	}
}
...
{	
    ...
    Mix_OpenAudio( 46300, MIX_DEFAULT_FORMAT, 2, 4096 );
#ifdef _WINDOWS
	glewInit();
#endif
    ...
    sheet = LoadTexture(RESOURCE_FOLDER"dirt-tiles.png");
	...
	while (!done) {
		...
		while (SDL_PollEvent(&event)) {
			if (event.type == SDL_QUIT || event.type == SDL_WINDOWEVENT_CLOSE) { done = true; }
                ...
                else if (event.key.keysym.scancode == SDL_SCANCODE_P && gState == GAME_STATE) {
                    sleep(5);
                }
                else if (event.key.keysym.scancode == SDL_SCANCODE_Q && gState == GAME_STATE) {
                    done = true;
                }
				...
				if (event.key.keysym.scancode == SDL_SCANCODE_Q && gState == GAME_OVER) { SDL_Quit();; }
                    ...
                    if (gState == TITLE_SCREEN) {
                        ...
                        lState = LEVEL_1;
                        levelInit(RESOURCE_FOLDER"map2.txt", vertexData, texCoordData); } //Level 1 Initilization
                    //Allows to skip through levels by pressing space. For presentation purposes
                    
                    
                    else if (gState == GAME_STATE){
                        if(lState == LEVEL_1){
                            levelInit(RESOURCE_FOLDER"map1.txt", vertexData, texCoordData);
                            lState = LEVEL_2;
                        }
                        else if (lState == LEVEL_2) {
                            levelInit(RESOURCE_FOLDER"map3.txt", vertexData, texCoordData);
                            lState = LEVEL_3;
                        }
                        else{
                            gState = TITLE_SCREEN;
		...
		glClearColor(0.52f, 0.44f, 1.0f, 1.5f);
		...
		case TITLE_SCREEN:
			write("Run Forrest Run!", -3.1f, 1.5f, 0.0f, .40f);
			...
			write("Find the door ", -2.70f, -.05f, 0.0f, 0.23f);
            write("Press P for 5 second pause", -2.70f, -.35f, 0.0f, 0.23f);
            write("Press Q to quit", -2.70f, -.65f, 0.0f, 0.23f);

			write("Press Space to Play!", -2.70f, -1.0f, 0.0f, .3f);
		...
		case GAME_OVER:
			...
			write(output, -1.75f, 1.2f, 0.0f, 0.5f);
			write("Press P to play again!", -3.05f, 0.4f, 0.0f, 0.28f, 0.0f);
            write("Or Press Q to quit!", -2.7f, -0.2f, 0.0f, 0.28f, 0.0f);
			viewMatrix.identity();
			program->setViewMatrix(viewMatrix);
			break;
		}
...
void Entity::Update(float elapsed) {
    ...
    scale_x = mapValue(fabs(velocity_y), 10.0, 0.0, 1.6, 1.0);
    if (entityType == PLAYER) scale_y = mapValue(fabs(velocity_y), 0.0, 10.0, 1.0, 2.0);
    else scale_y = mapValue(fabs(velocity_y), 0.0, 10.0, 1.0, 1.0);
	if (entityType == PLAYER) {
		...
		if (keys[SDL_SCANCODE_RIGHT] || keys[SDL_SCANCODE_D]) { acceleration_x = 1.0f; }
		else if (keys[SDL_SCANCODE_LEFT] || keys[SDL_SCANCODE_A]) { acceleration_x = -1.0f; }
	}
    if(entityType == ENEMY){
	...
	if (entity->entityType == GOAL) {
		...
		if (lState == LEVEL_1) {
			if (won) {
				lState = LEVEL_2;
				won = false; 
                levelInit(RESOURCE_FOLDER"map1.txt", vertexData, texCoordData); //Level 2 Initilization
 			}
		}
		if (lState == LEVEL_2) {
			if (won) {
				lState = LEVEL_3;
				won = false;
                levelInit(RESOURCE_FOLDER"map3.txt", vertexData, texCoordData); //Level 3 Initilization
			}
		}
		if (lState == LEVEL_3) {
...
bool Entity::isSolidTile(int index) {
	if (lState == LEVEL_1 || lState == LEVEL_3) {
		return (index == 0 || index == 63 || index == 8 || index == 78 || index == 187 || index == 209
                || index == 233 || index == 232 || index == 208) ? false : true;
	}
	return (index == 32 || index == 63 || index == 51 || index == 274 || index == 275 || index == 299 ||
